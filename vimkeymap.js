(function (mod) {
	...
	function initVim$1(CodeMirror) {
		...
		function getOption(name, cm, cfg) {
			...
			if (option.callback) {
				...
				var local =
					scope !== "global" && cm && cm.state.vim.options[name];
			...
			if (c) {
				...
				return {
					...
					expectCursorActivityForChange:
						c.expectCursorActivityForChange,
				};
			}
			return {
				// Change list
		...
		var vimApi = {
			...
			mapclear: function (ctx) {
				...
				var actualLength = defaultKeymap.length,
					origLength = defaultKeymapLength;
				var userKeymap = defaultKeymap.slice(
					0,
					actualLength - origLength
				);
		...
		var commandDispatcher = {
			...
			processSearch: function (cm, vim, command) {
				...
				function onPromptKeyUp(e, query, close) {
					...
					if (parsedQuery) {
						cm.scrollIntoView(
							findNext(cm, !forward, parsedQuery),
							30
						);
					} else {
						clearSearchHighlight(cm);
						cm.scrollTo(
							originalScrollPos.left,
				...
				switch (command.searchArgs.querySrc) {
					case "prompt":
						...
						if (macroModeState.isPlaying) {
							var query =
								macroModeState.replaySearchQueries.shift();
							handleQuery(
								query,
								true /** ignoreCase */,
								false /** smartCase */
			...
			evalInput: function (cm, vim) {
				...
				if (inputState.selectedCharacter) {
					...
					motionArgs.selectedCharacter =
						operatorArgs.selectedCharacter =
				...
				if (motion) {
					...
					if (vim.visualMode) {
						...
						updateMark(
							...
							vim,
							"<",
							cursorIsBefore(newAnchor, newHead)
								? newAnchor
								: newHead
						);
						updateMark(
					...
					if (vim.visualMode) {
						...
						if (linewise) {
							...
							if (mode == "block") {
								...
								ranges[0].head = new Pos(
									ranges[0].head.line + 1,
									0
								);
							}
						}
		...
		var motions = {
			...
			moveToBottomLine: function (cm, _head, motionArgs) {
				var line =
					getUserVisibleLines(cm).bottom - motionArgs.repeat + 1;
				return new Pos(
					line,
					findFirstNonWhiteSpaceCharacter(cm.getLine(line))
				);
			...
			moveByScroll: function (cm, head, motionArgs, vim) {
				...
				if (!repeat) {
					repeat =
						scrollbox.clientHeight / (2 * cm.defaultTextHeight());
				}
			...
			moveToLineOrEdgeOfDocument: function (cm, _head, motionArgs) {
				...
				if (motionArgs.repeatIsExplicit) {
					lineNum =
						motionArgs.repeat - cm.getOption("firstLineNumber");
				}
				return new Pos(
					lineNum,
			...
			textObjectManipulation: function (cm, head, motionArgs, vim) {
				...
				if (mirroredPairs[character]) {
					...
					if (
						head.ch > 0 &&
						isEndOfSentenceSymbol(content[head.ch])
					) {
						head.ch -= 1;
					}
					var end = getSentence(
			...
			repeatLastCharacterSearch: function (cm, head, motionArgs) {
				...
				var increment =
					(lastSearch.increment ? 1 : 0) * (forward ? -1 : 1);
			...
			scroll: function (cm, actionArgs, vim) {
				...
				if (actionArgs.forward) {
					...
					if (newBottom < cursorCoords.bottom) {
						cursor.line -=
							(cursorCoords.bottom - newBottom) / lineHeight;
			...
			paste: function (cm, actionArgs, vim) {
				...
				if (vim.visualMode) {
					...
					if (blockwise) {
						...
						selectBlock(
							cm,
							new Pos(cur.line + text.length - 1, cur.ch)
						);
						cm.replaceSelections(text);
						curPosFinal = cur;
					} else {
		...
		function commandMatch(pressed, mapped) {
			if (mapped.slice(-11) == "<character>") {
				...
				return pressedPrefix == mappedPrefix &&
					pressed.length > prefixLen
					? "full"
		...
		function getIndex(ranges, cursor, end) {
			for (var i = 0; i < ranges.length; i++) {
				var atAnchor =
					end != "head" && cursorEqual(ranges[i].anchor, cursor);
				var atHead =
					end != "anchor" && cursorEqual(ranges[i].head, cursor);
				if (atAnchor || atHead) {
					return i;
				}
			}
   ...
   */
			function nextChar(cm, idx) {
				if (
					idx.pos + idx.dir < 0 ||
					idx.pos + idx.dir >= idx.line.length
				) {
					idx.ln += idx.dir;
					if (!isLine(cm, idx.ln)) {
						idx.line = null;
        ...
        */
				...
				for (var i = 0; i < repeat; i++) {
					...
					if (i == 0 && found && cursorEqual(cursor.from(), pos)) {
						...
						if (
							...
							if (
								cm.getLine(lastEndPos.line).length ==
								lastEndPos.ch
							)
								found = cursor.find(prev);
						}
		...
		ExCommandDispatcher.prototype = {
			...
			parseInput_: function (cm, inputStream, result) {
				...
				// Parse command name.
				var commandMatch = inputStream.match(
					/^(\w+|!!|@@|[!#&*<=>@~])/
				);
				if (commandMatch) {
			...
			registers: function (cm, params) {
				...
				if (!regArgs) {
					...
					for (var i = 0; i < regArgs.length; i++) {
						...
						var register =
							registers[registerName] || new Register();
						regInfo +=
							'"' +
							registerName +
							"    " +
			...
			delmarks: function (cm, params) {
				...
				while (!stream.eol()) {
					...
					if (stream.match("-", true)) {
						...
						if (
							(isLowerCase(startMark) &&
								isLowerCase(finishMark)) ||
		...
		function executeMacroRegister(cm, vim, macroModeState, registerName) {
			...
			macroModeState.replaySearchQueries =
				register.searchQueries.slice(0);
			...
			if (
				...
				cm.operation(function () {
					...
					cm.forEachSelection(function () {
						...
						var anchor = cm.getCursor("anchor");
						var headOffset = !cursorIsBefore(head, anchor) ? -1 : 0;
						var anchorOffset = cursorIsBefore(head, anchor)
							? -1
							: 0;
						head = offsetCursor(head, 0, headOffset);
						anchor = offsetCursor(anchor, 0, anchorOffset);
